...
def main():
    ...
    topics_df = pd.read_csv('C:/Users/gabri/Downloads/Topics.csv')  # Ensure this CSV has a column named 'topic'
    ...
    for idx, row in topics_df.iterrows():
        topic = f"Propose some solutions, list the pros and cons of each, and select the best one for the following:: {row['topic']}"
        ...
        message_history = []
        for agent in [positive_debater, negative_debater, moderator, judge]:
            history = agent.get_message_history()
        if not history:
           print(f"Warning: No messages found for agent {agent.name}")
        for msg in history:
           message_history.append(f"{agent.name}: {msg.content}")
        ...
        result = {
            'topic': row['topic'],  # Save the original topic for reference
            'full_prompt': topic,

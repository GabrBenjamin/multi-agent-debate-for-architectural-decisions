...
def initialize_agents(config):
    ...
    meta_prompt = config['player_meta_prompt']
    player_meta_prompt = meta_prompt.replace('##debate_topic##', topic)
    aff_meta_prompt = player_meta_prompt.replace('##role##', 'Senior software engineer')
    neg_meta_prompt = player_meta_prompt.replace('##role##', 'Tester')
    moderator_meta_prompt = config['moderator_meta_prompt'].replace('##debate_topic##', topic)

    # Set system messages
    positive_debater.set_system_message(aff_meta_prompt)
    negative_debater.set_system_message(neg_meta_prompt)
...
def main():
    ...
    topic = "Choose one of the options considered for solving the following: Context and Problem Statement We have to provide a search bar to perform full-text search on ADRs. Decision Drivers Works in preview mode AND in the statically built version Provides good fuzzy search and stemming capabilities Is fast enough to be able to show results while typing Does not consume too much CPU and RAM on the client-side, especially for the statically built version Considered Options Option 1: Fuse.js Option 2: Lunr.js"

...
`Comparison.py` is optional aggregate-review assistance. It does not replace
...
`Main.py` currently assigns `llm_name="llama"`, but the agent routing does not
connect that name to `Utils/LLM.py`. Change it to a supported OpenAI or Together
model, or update the routing, before expecting this entry point to work.

## Limitations

- The structured JSON is generated by a model and can fail validation.
- `Run_all.py` fills missing impacts with neutral values before scoring.
- Model names, prompts, and dataset filenames are configured directly in code.
- API calls may incur provider charges.

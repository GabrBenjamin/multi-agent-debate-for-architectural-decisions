...
def retrieve(context, state, config):
    model = state['retriever_info']["model"]
    collection_name = state['retriever_info']["collection_name"]
    persist_directory = state['retriever_info']["persist_directory"]
    ...
    retrieved_docs = vector_store.similarity_search(context)
    retrieved_text = "\n\n".join(doc.page_content for doc in retrieved_docs)
    context = context + config['retriever_header'] + retrieved_text
...
def affirmative_opening(state: DebateState):
    ...
    prompt, state = retrieve(prompt, state, config)
...
def positive_debater_turn(state: DebateState):
    ...
    if state['retriever_mode'] == 'continuous':
        prompt, state = retrieve(prompt, state, config)
...
def judge_verdict(state: DebateState):
    ...
    state['messages'].append(HumanMessage(content=prompt1))
    ...
    state['messages'].append(HumanMessage(content=prompt2))

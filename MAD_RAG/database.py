...
class Database:
    ...
    def add_column(self, table_name, column):
        alter_table_sql = f"ALTER TABLE {table_name} ADD COLUMN IF NOT EXISTS {column};"
        self.db.execute_sql(alter_table_sql)
    ...
    def cache(self, item, table_name):
        ...
        if item_memory >= 1e6 or len(self.data) > 20000:
    ...
    def save(self):
        ...
        # print(f"Processing {len(self.data)} data entries")
        
        # Data grouping phase
        for idx, (data, table_name) in enumerate(self.data):
            # print(f"\nProcessing item {idx+1}/{len(self.data)} for table '{table_name}'")
            # print(f"Data type: {type(data).__name__}")
            
            if table_name not in group:
                # print(f"| Creating new group for table '{table_name}'")
                group[table_name] = []
                
            if isinstance(data, list):
                # print(f"| Extending with list of {len(data)} items")
                data = [data_ for data_ in data if len(data_) !=0 and isinstance(data_, dict)]
                group[table_name].extend(data)
                
            elif isinstance(data, pd.DataFrame):
                # print(f"| Converting DataFrame ({data.shape[0]} rows, {data.shape[1]} cols) to dict")
                converted = data.to_dict(orient='records')
                # print(f"| Adding {len(converted)} converted rows")
                group[table_name].extend(converted)
                
            elif isinstance(data, dict):    
                # print(f"| Appending single item of type {type(data).__name__}")
                if len(data) !=0:
        ...
        with self.engine.connect() as conn:
            ...
            # # print("Setting SQLite performance pragmas")
            ...
            try:
                for table_idx, (table_name, values) in enumerate(group.items()):
                    if len(values) == 0:
                        continue
                    # print(f"\nProcessing table {table_idx+1}/{len(group)}: '{table_name}'")
                    # print(f"Total rows to insert: {len(values)}")
                    ...
                    # print(f"| SQLite max params: {max_params}, columns: {len(columns)}")
                    ...
                    # print(f"| Will insert in {total_chunks} chunks")
                    for i in range(0, len(values), chunk_size):
                        ...
                        # print(f"| First parameter sample: {params[0] if params else 'None'}")
                        
                        try:
                            self._execute_batch(conn, insert_sql, params)
                            
                            # print(f"| Successfully executed chunk")
                        except Exception as e:
                ...
                pass
                # print("\nResetting SQLite pragmas to defaults")
                # conn.execute(text("PRAGMA synchronous = NORMAL"))
                # conn.execute(text("PRAGMA journal_mode = DELETE"))

        # print("\n=== Cleanup phase ===")
        # print(f"Clearing {len(self.data)} data entries")
        self.data = []
    ...
    def to_df(self, table_name, filter_dict=None):
        if not self.table_exists(table_name):
            return pd.DataFrame()
        table = Table(table_name, self.metadata, autoload_with=self.engine)
        stmt = select(table)

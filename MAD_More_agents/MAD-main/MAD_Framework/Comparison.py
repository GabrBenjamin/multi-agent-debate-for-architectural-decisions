...
# We'll inject {row_id}, {context_considered_drivers}, and {debate_answer} into this template.
template = """You are a helpful assistant. 
We have two decisions:
1) context_considered_drivers: {context_considered_drivers}
...
prompt = PromptTemplate(
    input_variables=["row_id", "context_considered_drivers", "debate_answer"],
    template=template
)
...
for idx, row in df.iterrows():
    ...
    response = chain.run({
        "row_id": row_id,
        "context_considered_drivers": context_considered_drivers,

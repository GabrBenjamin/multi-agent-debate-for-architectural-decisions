...
def main():
    ...
    topic = "Propose some solutions, list the pros and cons of each, and select the best one for the following: Context and Problem Statement We have to provide a search bar to perform full-text search on ADRs. Decision Drivers Works in preview mode AND in the statically built version Provides good fuzzy search and stemming capabilities Is fast enough to be able to show results while typing Does not consume too much CPU and RAM on the client-side, especially for the statically built version"
    config['debate_topic'] = topic

    # Initialize agents
    positive_debater, negative_debater, third_part, moderator, judge = initialize_agents(config)

...
def main():
    ...
    init = f"""## Context and Problem Statement Which language version should we prefer? Should we restrict the version at all? ## Decision Drivers * Nullable reference types * Source compatible with Unity 2018.4 * Binary compatible with Unity 2018.4 * Source compatible with Unity 2020.3 * Source compatible with online coding platforms like CodinGame or LeetCode ## Considered Options * Latest available language version * C# 7.3 * C# 8 * C# 9"""
    promptone = f"""You are an expert in software architeture. Collect Scenarios[eliciting sys tem usage scenarios and collecting requirements, constraints, and environmental details ] for the following : {init} """
    ...
    topic = f"""Decise the best proposed solution from {evaluatedscenarios}"""  
